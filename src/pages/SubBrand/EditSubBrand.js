...
const EditSubBrand = () => {
    ...
    const initValue = {
        brandId: location?.state?.singleData?.Brand?.[0]?._id,
        name: location?.state?.singleData?.name,
        type: location?.state?.singleData?.type,
        image: location?.state?.singleData?.logoUrl
    }
    const [formValue, setFormValue] = useState(initValue);
    const [imageLoader, setImgLoader] = useState(false);
    const [brandData, setBrandData] = useState([]);

    console.log("location", location?.state, formValue)
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        const res = await HttpClient.requestData("update-subbrand/" + location?.state?.singleData?._id, "PUT", data);
        console.log("resCat", res)
        if (res && res?.status) {
            toast.success("Sub-Brand Updated Successfully");
            // setFormValue(initValue);
            navigate('/manage-sub-brand');
            setIsLoading(false);
        } else {
    ...
    return (
        <Box m="20px">
            ...
            <Header title="EDIT SUB-BRAND" subtitle="" />
